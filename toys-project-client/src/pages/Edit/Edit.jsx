...
const Edit = () => {
  ...
  console.log(data);
  useEffect(() => {
    fetch(`http://localhost:5000/allToys/${data.id}`)
      .then((res) => res.json())
      .then((data) => {
        setInitial(data);
      });
  ...
  const handleAddToy = (event) => {
    ...
    fetch(`http://localhost:5000/allToys/${initial._id}`, {
      method: "PUT",
      headers: {
        ...
        <Row>
          <Col md={6}>
            <Form
              ...
              className="bg-info bg-opacity-25 rounded p-4 mt-5 mb-5"
            >
